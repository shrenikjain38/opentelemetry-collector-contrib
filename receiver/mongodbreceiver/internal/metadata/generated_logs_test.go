...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordDbServerTopQueryEvent(ctx, timestamp, "db.collection.name-val", "db.namespace-val", "db.query.text-val", AttributeDbSystemNameMongodb, 26.100000, 31, 31, 26.100000, "mongodb.operation.explain_plan-val", "mongodb.operation.explain_plan_hash-val", 31, "mongodb.operation.plan.summary-val", 33, "mongodb.operation.type-val")
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample":
					...
					attrVal, ok = lr.Attributes().Get("mongodb.client.app.name")
					assert.True(t, ok)
					assert.Equal(t, "mongodb.client.app.name-val", attrVal.Str())
					...
					attrVal, ok = lr.Attributes().Get("mongodb.operation.state")
					assert.True(t, ok)
					...
					attrVal, ok = lr.Attributes().Get("mongodb.operation.yield.count")
					assert.True(t, ok)
				...
				case "db.server.top_query":
					...
					assert.True(t, ok)
					assert.EqualValues(t, 31, attrVal.Int())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.docs_returned")
					assert.True(t, ok)
					assert.EqualValues(t, 31, attrVal.Int())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.duration")
					assert.True(t, ok)
					assert.Equal(t, 26.100000, attrVal.Double())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.explain_plan")
					assert.True(t, ok)
					assert.Equal(t, "mongodb.operation.explain_plan-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.explain_plan_hash")
					assert.True(t, ok)
					assert.Equal(t, "mongodb.operation.explain_plan_hash-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.keys_examined")
					assert.True(t, ok)
					assert.EqualValues(t, 31, attrVal.Int())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.plan.summary")
					assert.True(t, ok)
					assert.Equal(t, "mongodb.operation.plan.summary-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.response_length")
					assert.True(t, ok)
					assert.EqualValues(t, 33, attrVal.Int())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.type")
					assert.True(t, ok)
